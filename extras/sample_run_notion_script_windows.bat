...
:: Function to log messages with timestamps (using PowerShell)
:log
set "msg=%~1"
for /f "usebackq tokens=*" %%a in (`powershell -NoProfile -Command "Get-Date -Format 'yyyy-MM-dd HH:mm:ss'"`) do set "log_time=%%a"
echo %log_time% - %msg%
goto :eof
